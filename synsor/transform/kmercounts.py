...
def toEmbedding(kmerDf, how='pca', n_components=3, **kwargs):
    ## Try to load everything into memory.
    ## SparkML just doesn't seem to be as good as sklearn
    (kmerId, kmerCount) = toDriver(kmerDf)
    ...
    if (how == 'pca'):
        ...
        ## Run PCA
        m     = decomposition.PCA(n_components, **kwargs)
        cols  = [f'Component{i}' for i in range(1, n_components + 1)]
        compDf = pd.DataFrame(m.fit_transform(kmerCount), columns=cols)
        compDf = pd.concat([kmerId, compDf], axis=1)

        ## Calculate variance and loadings
        variances = map(lambda x: str(round(x*100, 2)), m.explained_variance_ratio_)
        variances = {c:v for c,v in zip(cols, variances)}
        loadings  = m.components_.T * np.sqrt(m.explained_variance_)
        loadings  = pd.DataFrame(loadings, columns=cols)

        ## Load the table back to Spark
        ss = SparkSession.getActiveSession()
        compDf = ss.createDataFrame(compDf)
        return (compDf, variances, loadings)

    elif (how == 'tsne'):
        ## tSNE calculates the similarity between pairs of vectors in both the
        ## high and low low dimensional space, and optimising these two
        ## similarity measures using a cost function (KL-divergence)
        ##
        ## Similarity in the high dimensional space is measured by its density
        ## and probabilities using the Gaussian distribution. Vectors that
        ## are similar have similar probabilities. The distribution varies
        ## according to the perplexity.
        ##
        ## Similarity in low dimensional space is measured using a
        ## Student t-distribution (i.e., Cauchy distribution).
        ##
        ## The above probability distributions are then compared by measuring the
        ## KL divergence. Low KL divergence indicates that the two distributions
        ## are similar. However, unlike distance metrics, KL divergence is not
        ## a symmetric measure.
        ##
        ...
        ## Both MDS and PCA appear to represent global differences well, but
        ## don't represent local differences very well.
        ##
        ## tSNE is good for preserving local/relative structure and therefore
        ## generates informative visualisations of the heterogeneity in the
        ## data. However, it does not guarantee that inter-class distances
        ## are preserved correctly, making it difficult for us cluster and
        ## understand something about relatedness between clusters.
        from sklearn import manifold

        ## Run TSNE
        m     = manifold.TSNE(n_components, **kwargs)
        cols  = [f'Component{i}' for i in range(1, n_components + 1)]
        compDf = pd.DataFrame(m.fit_transform(kmerCount), columns=cols)
        compDf = pd.concat([kmerId, compDf], axis=1)
        ...
        raise NotImplementedError("Unknown embedding method.")
